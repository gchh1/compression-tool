...
3. gui/core/training_store.py
   └── TrainingSampleV3 新增探索字段 (向后兼容, 默认值)

4. gui/core/engine.py
...
| 模块 | 硬编码位置 |
|------|-----------|
| 主窗口菜单/工具栏 | `gui/widgets/main_window.py` |
| 文件表格 | `gui/widgets/main_window.py` FileTableWidget |
| 决策详情对话框 | `gui/widgets/main_window.py` DecisionDetailDialog |
| 状态栏 | `gui/widgets/main_window.py` StatusBarWidget |
| 算法选择器 | `gui/widgets/main_window.py` AlgorithmSelector |
| 算法配置对话框 | `gui/widgets/algorithm_config_dialog.py` |
| 对比视图 | `gui/widgets/comparison_view.py`, `heatmap_widgets.py` |
