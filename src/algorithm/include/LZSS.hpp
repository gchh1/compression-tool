...
class LZSS {
   public:
    static std::vector<uint8_t> compress(const std::vector<uint8_t>& input, size_t dictionary_buffer_size = 4095, size_t min_match_length = 3, bool use_flag_encoding = true);
    static std::vector<uint8_t> decompress(const std::vector<uint8_t>& input, size_t min_match_length = 3, bool use_flag_encoding = true);
