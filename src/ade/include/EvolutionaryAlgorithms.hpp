...
struct ParameterBounds {
    ...
    auto clamp(AlgorithmParams& params) const -> void {
        params.window_size = std::clamp(params.window_size,
                                        window_size_min, window_size_max);
        params.min_match = std::clamp(params.min_match,
                                      min_match_min, min_match_max);
        params.max_chain_length = std::clamp(params.max_chain_length,
                                             max_chain_min, max_chain_max);
        params.lookahead_size = std::clamp(params.lookahead_size,
                                           lookahead_min, lookahead_max);
        params.dp_range = std::clamp(params.dp_range,
                                     dp_range_min, dp_range_max);
    }
    ...
    auto to_vector(const AlgorithmParams& params) const -> std::vector<double> {
        return {
            static_cast<double>(params.window_size),
...
public:
    ...
    auto optimize(FitnessFunction fitness,
        ...
        size_t elite_count = std::max(size_t(1),
            static_cast<size_t>(config.population_size * config.elitism_ratio));
        ...
        switch (choice) {
            case 0: {
                ...
                auto delta = static_cast<int64_t>(
                    (real_dist_(rng_) - 0.5) * 2 * range * mutation_strength);
...
public:
    ...
    auto optimize(FitnessFunction fitness,
        ...
        for (size_t iter = 0; iter < config.max_iterations; ++iter) {
            ...
            double w = config.inertia_weight *
                       std::pow(config.inertia_decay, static_cast<double>(iter));
            ...
            for (auto& particle : swarm) {
                ...
                for (size_t d = 0; d < NUM_PARAMS; ++d) {
                    ...
                    particle.velocity[d] = std::clamp(
                        particle.velocity[d], -vmax, vmax);
            ...
            if (std::abs(global_best_fitness - prev_global_best) <
...
private:
    ...
    auto compute_velocity_bound(size_t dim, const ParameterBounds& bounds) -> double {
        switch (dim) {
            case 0: return (bounds.window_size_max - bounds.window_size_min) * 0.25;
            case 1: return (bounds.min_match_max - bounds.min_match_min) * 0.25;
            case 2: return (bounds.max_chain_max - bounds.max_chain_min) * 0.25;
            case 3: return (bounds.lookahead_max - bounds.lookahead_min) * 0.25;
            case 4: return (bounds.dp_range_max - bounds.dp_range_min) * 0.25;
...
public:
    ...
    auto optimize(FitnessFunction fitness,
        ...
        double w_sum = 0.0, w_pos_sum = 0.0;
        for (size_t i = 0; i < mu; ++i) {
            if (weights[i] > 0) w_pos_sum += weights[i];
        ...
        double cmu = std::min(1.0 - c1, 2.0 * (mueff - 2.0 + 1.0 / mueff) /
                              ((N + 2.0) * (N + 2.0) + mueff));
        double damps = 1.0 + 2.0 * std::max(0.0, sqrt((mueff - 1.0) / (N + 1.0)) - 1.0) + cs;
        double chiN = sqrt(static_cast<double>(N)) * (1.0 - 1.0 / (4.0 * N) + 1.0 / (21.0 * N * N));

        size_t eigen_eval = 0;
        std::vector<double> D(N, 1.0), inv_D(N, 1.0);
        ...
        for (size_t iter = 0; iter < config.max_iterations; ++iter) {
            ...
            for (size_t d = 0; d < N; ++d) ps[d] = (1.0 - cs) * ps[d] +
                sqrt(cs * (2.0 - cs) * mueff) * diff[d];
            ...
            double hsig = 0.0;
            if (ps_norm / sqrt(1.0 - pow(1.0 - cs, 2.0 * (iter + 1))) / chiN <
                1.4 + 2.0 / ((double)(N + 1))) {
                hsig = 1.0;
            }

            for (size_t d = 0; d < N; ++d) pc[d] = (1.0 - cc) * pc[d] +
                hsig * sqrt(cc * (2.0 - cc) * mueff) * diff[d];
            ...
            sigma *= exp((cs / damps) * (ps_norm / chiN - 1.0));
...
private:
    ...
    auto rotate(std::vector<std::vector<double>>& C,
        ...
        std::vector<double> Cp = C[p], Cq = C[q];
        for (size_t i = 0; i < N; ++i) {
            C[p][i] = c * Cp[i] + s * Cq[i];
            C[q][i] = -s * Cp[i] + c * Cq[i];
        }

        std::vector<double> Bp = B[p], Bq = B[q];
        for (size_t i = 0; i < N; ++i) {
            B[i][p] = c * Bp[i] + s * Bq[i];
            B[i][q] = -s * Bp[i] + c * Bq[i];
        }
...
public:
    ...
    ParameterOptimizer() : algorithm_(Algorithm::NONE) {}

    auto set_algorithm(Algorithm algo) -> void { algorithm_ = algo; }

    auto get_algorithm() const -> Algorithm { return algorithm_; }
    ...
    auto to_json() const -> std::string {
        ...
        json << "  \"algorithm_name\": \""
             << algorithm_to_string(algorithm_) << "\"\n";
        json << "}\n";
        return json.str();
    }
    ...
    static auto algorithm_to_string(Algorithm algo) -> std::string {
        switch (algo) {
            case Algorithm::GENETIC_ALGORITHM: return "GeneticAlgorithm";
            case Algorithm::PARTICLE_SWARM: return "ParticleSwarmOptimization";
            case Algorithm::CMA_ES: return "CMA-ES";
            case Algorithm::NONE: default: return "None";
        }
    }
