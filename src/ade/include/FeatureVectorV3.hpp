...
/**
 * @brief Output stream operator for FileType enum (for testing/logging)
 */
inline auto operator<<(std::ostream& os, FileType type) -> std::ostream& {
...
/**
 * @brief Convert ExtensionType enum to human-readable string
 */
inline auto extension_type_to_string(ExtensionType type) -> std::string {
    switch (type) {
...
/**
 * @brief Output stream operator for ExtensionType enum (for testing/logging)
 */
inline auto operator<<(std::ostream& os, ExtensionType type) -> std::ostream& {
    os << extension_type_to_string(type);
    return os;
}

/**
 * @brief Map FileType to corresponding ExtensionType
 *
 * This routing function determines which Extension segment to extract
 * based on the detected file type from Magic Bytes analysis.
 */
inline auto map_file_type_to_extension(FileType file_type) -> ExtensionType {
    switch (file_type) {
        // Text/Code → TextExtension
        ...
        // Everything else → BinaryExtension
        default:
...
struct BaseSegment {
    ...
    /**
     * @brief Zero-initialize all features
     */
    auto reset() -> void { std::memset(this, 0, SIZE); }

    /**
     * @brief Access feature by index with bounds checking
     */
...
struct FeatureVectorV3 {
    ...
    /**
     * @brief Get total serialized size in bytes
     */
    ...
    /**
     * @brief Convert to flat array for ML consumption (with zero-padding)
     *
     * Pads unused extension slots with zeros to create a fixed-size input vector.
     * Default target is 33 dimensions (20 base + max 8 extension + 5 padding).
     *
     * @param target_dim Target dimensionality (default: 33)
     * @return Fixed-size float vector ready for ML model input
     */
    auto to_padded_array(size_t target_dim = 33) const -> std::vector<float> {
        std::vector<float> result(target_dim, 0.0f);

        // Copy Base Segment (20 dimensions)
        ...
        // Remaining positions already initialized to 0 (implicit padding)

        return result;
    }

    /**
     * @brief Reset entire vector to zeros
     */
    auto reset() -> void {
        base.reset();
    ...
    /**
     * @brief Validate vector integrity
     * @return True if extension dimension matches expected value for type
     */
