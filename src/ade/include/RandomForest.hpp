...
private:
    ...
    auto majority_class(const std::vector<size_t>& indices) const -> int {
        ...
        return static_cast<int>(std::distance(counts.begin(),
        ...
        for (size_t f = 0; f < features_to_try; ++f) {
            ...
            for (size_t i = 0; i < sorted.size() - 1; ++i) {
                ...
                double weighted_gini =
                    (left_total * left_gini + right_total * right_gini) /
    ...
    auto node_to_json(std::ostringstream& json, const Node& node) const -> void {
        if (node.is_leaf) {
            json << "{\"leaf\": true, \"class\": " << node.predicted_class
                 << ", \"dist\": [";
            ...
            json << "{\"leaf\": false, \"feature\": " << node.feature_index
                 ...
                 << ", \"left\": ";
            if (node.left) node_to_json(json, *node.left);
            else json << "null";
            json << ", \"right\": ";
            if (node.right) node_to_json(json, *node.right);
            else json << "null";
            json << "}";
        }
    }
    ...
    auto load_node(std::istream& is, Node& node) -> void {
        ...
        if (node.is_leaf) {
            ...
            is.read(reinterpret_cast<char*>(node.class_distribution.data()),
                    dist_size * sizeof(float));
        } else {
            uint64_t fi = 0;
...
public:
    ...
    auto load(std::istream& is) -> bool {
        ...
        uint64_t cfg_max_depth = 0, cfg_min_split = 0, cfg_min_leaf = 0;
        uint64_t cfg_max_feat = 0, cfg_seed = 0;
...
private:
    ...
    auto bootstrap_sample(const std::vector<TrainingSample>& samples,
        ...
        size_t sample_size = static_cast<size_t>(
            samples.size() * config.bootstrap_ratio);
