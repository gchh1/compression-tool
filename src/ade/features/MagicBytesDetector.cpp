...
auto MagicBytesDatabase::initialize() -> std::vector<MagicSignature> {
...
auto MagicBytesDatabase::get_signatures() -> const std::vector<MagicSignature>& {
    static const std::vector<MagicSignature> signatures = initialize();
    return signatures;
}

MagicBytesDetector::MagicBytesDetector() : signatures_(MagicBytesDatabase::get_signatures()) {}

auto MagicBytesDetector::detect(const uint8_t* data, size_t size) const -> DetectionResult {
...
auto MagicBytesDetector::detect(const std::vector<uint8_t>& data) const -> DetectionResult {
    return detect(data.data(), data.size());
}

auto MagicBytesDetector::get_type_description(FileType type) -> std::string {
...
auto MagicBytesDetector::matches_signature(const uint8_t* data, size_t size,
                                           const MagicSignature& sig) const -> bool {
    if (size < sig.offset + sig.pattern.size()) {
        return false;
    }
    return std::memcmp(data + sig.offset, sig.pattern.data(), sig.pattern.size()) == 0;
}

auto MagicBytesDetector::apply_special_rules(const uint8_t* data, size_t size,
...
auto MagicBytesDetector::detect_by_heuristics(const uint8_t* data, size_t size) const
    -> DetectionResult {
    DetectionResult result;
    result.type = FileType::UNKNOWN;
    ...
    float printable_ratio = static_cast<float>(printable_count) / static_cast<float>(sample_size);
    float whitespace_ratio = static_cast<float>(whitespace_count) / static_cast<float>(sample_size);
