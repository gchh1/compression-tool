...
auto algorithm_id_to_string(AlgorithmID id) -> std::string {
    switch (id) {
        ...
        case AlgorithmID::LZDP: return "LZDP";
...
auto AlgorithmParams::to_string() const -> std::string {
    std::ostringstream s;
...
auto CompressionDecision::to_json() const -> std::string {
...
auto AlgorithmDecisionEngine::set_mode(Mode mode) -> void {
    mode_ = mode;
}

auto AlgorithmDecisionEngine::get_mode() const -> Mode {
    return mode_;
}

auto AlgorithmDecisionEngine::is_ml_ready() const -> bool {
    return rf_trained_;
}

auto RuleEngine::decide(const FeatureVectorV3& features) const -> CompressionDecision {
...
auto RuleEngine::decide_with_explanation(const FeatureVectorV3& features) const
...
auto RuleEngine::is_already_compressed(const FeatureVectorV3& features) const -> bool {
    const auto& base = features.base;
    float entropy = base.shannon_entropy;
    float unique_byte = base.unique_byte_ratio;
...
auto RuleEngine::decide_low_entropy(const FeatureVectorV3& features) const -> CompressionDecision {
    CompressionDecision decision;
    const auto& base = features.base;
...
auto RuleEngine::decide_medium_entropy(const FeatureVectorV3& features) const -> CompressionDecision {
    CompressionDecision decision;
    const auto& base = features.base;
    ...
    if (is_text) {
        ...
        if (base.dict_potential < 0.5f) {
            decision.algorithm = AlgorithmID::LZDP;
            decision.params.window_size = 8192;
            decision.params.lookahead_size = 256;
            decision.params.dp_range = 3;
            decision.estimated_ratio = 0.45f;
            decision.confidence = 0.70f;
            decision.reason = "Mixed content + dict potential → LZDP with DP matching";
        } else {
...
auto RuleEngine::decide_high_entropy(const FeatureVectorV3& features) const -> CompressionDecision {
    CompressionDecision decision;
    const auto& base = features.base;
    ...
    if (base.local_entropy_var > 0.5f) {
        ...
        decision.reason = "Uniform high entropy → LZDP fast mode, limited gain expected";
    }

    return decision;
}

auto RuleEngine::decide_very_high_entropy(const FeatureVectorV3& features) const
    -> CompressionDecision {
    CompressionDecision decision;
    const auto& base = features.base;
...
AlgorithmDecisionEngine::AlgorithmDecisionEngine()
    : mode_(Mode::RULE_BASED), rf_trained_(false) {}

auto AlgorithmDecisionEngine::train(const std::vector<TrainingSample>& samples,
...
auto AlgorithmDecisionEngine::decide_with_explanation(const FeatureVectorV3& features) const
    -> std::pair<CompressionDecision, std::string> {
    auto decision = decide(features);
...
auto AlgorithmDecisionEngine::feature_importance() const -> std::vector<float> {
    if (!rf_trained_) return {};
    return rf_.feature_importance();
}

auto AlgorithmDecisionEngine::to_json() const -> std::string {
    std::ostringstream json;
    json << "{\n";
...
auto AlgorithmDecisionEngine::save_model(const std::string& filepath) const -> bool {
    if (!rf_trained_) return false;
    return rf_.save(filepath);
}

auto AlgorithmDecisionEngine::load_model(const std::string& filepath) -> bool {
    bool ok = rf_.load(filepath);
    if (ok) rf_trained_ = true;
    return ok;
}

auto AlgorithmDecisionEngine::save_model(std::ostream& os) const -> bool {
    if (!rf_trained_) return false;
    return rf_.save(os);
}

auto AlgorithmDecisionEngine::load_model(std::istream& is) -> bool {
    bool ok = rf_.load(is);
    if (ok) rf_trained_ = true;
    return ok;
}
...
auto AlgorithmDecisionEngine::sample_from_result(const ExtractionResult& result,
...
auto AlgorithmDecisionEngine::default_params_for_algorithm(AlgorithmID algo) -> AlgorithmParams {
...
auto AlgorithmDecisionEngine::estimate_ratio(const FeatureVectorV3& features, AlgorithmID algo)
    -> float {
