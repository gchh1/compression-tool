...
auto StreamingCompressAdapter::flushOneChunk() -> bool {
    if (input_buffer_.size() < chunk_size_) return false;
...
auto StreamingCompressAdapter::flushFinalChunk() -> void {
...
auto StreamingCompressAdapter::emitChunk(const std::vector<uint8_t>& compressed) -> void {
...
auto StreamingCompressAdapter::emitTerminator() -> void {
    encodeU32BE(0, output_buffer_);
}

auto StreamingCompressAdapter::copyToOutput(std::span<uint8_t> write) -> size_t {
...
auto StreamingDecompressAdapter::copyToOutput(std::span<uint8_t> write) -> size_t {
    size_t available = output_buffer_.size() - output_pos_;
    size_t to_write = std::min(available, write.size());
    if (to_write > 0) {
        std::memcpy(write.data(), output_buffer_.data() + output_pos_, to_write);
