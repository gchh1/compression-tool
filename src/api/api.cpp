...
static core::ParamOverrides parse_overrides_json(const std::string& json) {
    core::ParamOverrides overrides;
    if (json.empty()) return overrides;
    auto j = nlohmann::json::parse(json, nullptr, false);
    if (j.is_discarded() || !j.is_object()) return overrides;
    for (auto& [k, v] : j.items()) {
        if (v.is_number_integer())
            overrides.values[k] = v.get<int>();
    }
    return overrides;
}
...
auto compress(const std::vector<uint8_t>& data,
    ...
    auto overrides = parse_overrides_json(overrides_json);

    std::vector<std::unique_ptr<algorithm::IAlgorithm>> algos;
    for (auto id : chain) {
        if (auto a = core::createAlgorithm(id, overrides.empty() ? nullptr : &overrides))
...
auto compressFile(const std::string& input_path,
    ...
    // Attach viz observer on first AlgorithmBase (e.g. DPFlate, not wrapped adapters)
    std::shared_ptr<compressor::viz::DiskVizObserver> viz_observer;
    if (!viz_path.empty()) {
        viz_observer =
    ...
    // Write WCX header synchronously to .part
    {
