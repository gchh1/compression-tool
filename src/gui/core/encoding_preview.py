"""Encoding / bit-width preview text for algorithm config (aligned with C++ LZDP::calcBitWidth)."""


def calc_bit_width(max_val: int) -> int:
    """Same as ``LZDP::calcBitWidth`` in ``src/algorithm/LZDP.cpp``."""
    if max_val <= 0:
        return 1
    bits = 0
    v = max_val
...
def lzdp_effective_min_match(offset_bits: int, length_bits: int, min_match_param: int) -> int:
    """Matches ``LZDP::get_min_match()`` when ``min_match_param == 0``."""
...
def format_lzdp_preview(
    ...
    lines = [
        ...
        + ("（参数为 0 时按 (offset_bits+length_bits)/8+1）" if min_match_param == 0 else f"（参数 = {min_match_param}）"),
...
def format_lzss_preview(search_size: int, lookahead_size: int, min_match_param: int) -> str:
    """LZSS token layout from ``src/algorithm/LZSS.cpp`` (flag groups of 8)."""
...
def format_dpflate_lz_reference_preview(
    ...
    """DPFlate output is Huffman-coded; show LZ-relevant field widths for comparison."""
    ob = calc_bit_width(search_size)
    lb = calc_bit_width(lookahead_size)
