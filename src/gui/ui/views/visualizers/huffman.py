...
class HuffmanTreeWidget(QWidget):
    ...
    def set_ll_tree(self, ll_lengths: list[int]) -> None:
        ...
        self._title = f"Huffman µáæ ΓÇö Literal/Length ({len(codes)} symbols)"
    ...
    def set_dist_tree(self, dist_lengths: list[int]) -> None:
        ...
        self._title = f"Huffman µáæ ΓÇö Distance ({len(codes)} symbols)"
    ...
    def set_block_index(self, idx: int) -> None:
        ...
        self._title = f"Huffman µáæ ΓÇö Block #{idx}"
        ...
        for sym, code in sorted(self._codes.items(), key=lambda x: (len(x[1]), x[1])):
            ...
            for bit in code[:-1]:
                ...
                if key not in node:
                    ...
                    # Invalid: prefix collision ΓÇö replace with internal node
        ...
        for x, y, is_leaf, sym, code in self._nodes:
            ...
            if sym < 256:
                ...
                if 0x20 <= sym < 0x7f:
                    ...
                    label = chr(sym) if sym != 0x20 else 'ΓÉú'
...
class HuffmanTreePanel(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._label = QLabel("Huffman µáæ")
...
def _darken_color(qc: QColor, amount: int) -> QColor:
    ...
    """Mix ``qc`` toward black; ``amount`` is roughly a percentage (0ΓÇô100)."""
...
def code_length_to_color(code_length: int, max_cl: int = 15) -> QColor:
    ...
    """τáüΘò┐ ΓåÆ Φë▓τ¢╕∩╝Üτƒ¡τáüσüÅτ╗┐(heatmap_low)∩╝îΘò┐τáüσüÅτ║ó(heatmap_high)πÇé"""
...
class HuffmanTreeCanvas(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if not self._node_positions:
            ...
            painter.drawText(QRect(0, 0, w, h), Qt.AlignmentFlag.AlignCenter, "µùá Huffman µáæµò░µì«")
...
class HuffmanFreqChart(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if not self._entries:
            ...
            painter.drawText(QRect(0, 0, w, h), Qt.AlignmentFlag.AlignCenter, "µùáτ╝ûτáüµò░µì«")
        ...
        painter.drawText(QRect(0, 0, w, margin_top), Qt.AlignmentFlag.AlignCenter, "τ¼ªσÅ╖ΘóæτÄçσêåσ╕â")
