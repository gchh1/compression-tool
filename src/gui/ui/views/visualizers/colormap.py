...
Provides the Viridis colormap as a 256-entry lookup table — perceptually
...
# ── Viridis colormap — 256 RGB triplets (0–255) ──────────────────────
# Source: matplotlib / BIDS colormap (CC0)
# Perceptually uniform sequential colormap: purple → teal → green → yellow
...
def viridis_color(t: float) -> QColor:
    """Map a normalised value *t* ∈ [0, 1] to a Viridis ``QColor``.

    Values outside [0, 1] are clamped.  Lookup is O(1) nearest-neighbour
    into the 256-entry LUT — zero allocations per call after warm-up.
    """
