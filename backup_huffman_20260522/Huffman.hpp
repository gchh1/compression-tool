...
public:
    ...
    void rebuildCodesFromLengths(const std::vector<std::pair<uint32_t, uint8_t>>& entries) {
        ...
        std::vector<int> bl_count(static_cast<size_t>(max_len) + 1, 0);//bl_count[i] 表示长度为 i 的符号数量
        for (auto& e : entries) bl_count[e.second]++;

        std::vector<uint64_t> next_code(static_cast<size_t>(max_len) + 1, 0);//next_code[i] 表示长度为 i 的下一个编码
        uint64_t code = 0;
